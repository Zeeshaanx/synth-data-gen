...
def create_job(bg: BackgroundTasks, generate_request: str = Form(...), seed_data: UploadFile = File(None)):
    try:
        ...
        db.insert_job(job_id, "create", req.model_provider, req.model_id, req.num_records or 50)
...
def preview_job(bg: BackgroundTasks, generate_request: str = Form(...), seed_data: UploadFile = File(None)):
    try:
        ...
        job_id = str(uuid.uuid4())
        JOB_STORE[job_id] = {"status": "processing", "created_at": time.time()}
        db.insert_job(job_id, "preview", req.model_provider, req.model_id, req.num_records or 50)
...
@router.get("/jobs")
def get_all_jobs():
    jobs = sorted([{"job_id": k, **v} for k, v in JOB_STORE.items()], key=lambda x: x["created_at"])
